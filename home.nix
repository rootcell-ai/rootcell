...
in
{
  ...
  programs.direnv = {
    ...
    stdlib = ''
      rootcell_ca_bundle=${caBundle}
      rootcell_ca_dir=${caDir}

      rootcell_export_ca_bundle() {
        if [ -r "$rootcell_ca_bundle" ]; then
          export NODE_EXTRA_CA_CERTS="$rootcell_ca_bundle"
          export NIX_SSL_CERT_FILE="$rootcell_ca_bundle"
          export SSL_CERT_FILE="$rootcell_ca_bundle"
          export SSL_CERT_DIR="$rootcell_ca_dir"
          export REQUESTS_CA_BUNDLE="$rootcell_ca_bundle"
          export CURL_CA_BUNDLE="$rootcell_ca_bundle"
          export GIT_SSL_CAINFO="$rootcell_ca_bundle"
          export PIP_CERT="$rootcell_ca_bundle"
          export AWS_CA_BUNDLE="$rootcell_ca_bundle"
          export UV_NATIVE_TLS=true
        fi
      }

      rootcell_wrap_direnv_loader() {
        local fn="$1"
        local original="rootcell_original_$fn"
        if declare -F "$fn" >/dev/null && ! declare -F "$original" >/dev/null; then
          eval "$(declare -f "$fn" | sed "1s/^$fn/$original/")"
          eval "$fn() { $original \"\$@\"; local status=\$?; rootcell_export_ca_bundle; return \$status; }"
        fi
      }

      rootcell_export_ca_bundle
      rootcell_wrap_direnv_loader use_flake
      rootcell_wrap_direnv_loader use_nix
    '';
  };
