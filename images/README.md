...
Official image asset releases are created from explicit tags named
`image-vYYYYMMDD.N`, for example `image-v20260515.1`. Create and push the tag
first, then run the `Release rootcell image assets` workflow with that exact
tag. The workflow checks out the tag, verifies that it exists, and passes
`--verify-tag` to `gh release create` so GitHub CLI cannot create an implicit
